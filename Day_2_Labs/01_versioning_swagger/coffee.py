...
def delete(coffee_name):
    """
    This function deletes a drink from the coffee object
    :param coffee_name:   name of drink to delete
    :return:        200 on successful delete, 404 if not found
    """
    # Does the drink to delete exist?
    if coffee_name in COFFEE:
        del COFFEE[coffee_name]
        return make_response(
            "{coffee_name} successfully deleted".format(coffee_name=coffee_name), 200
        )
    else:
        abort(
            404, "Drink with name {coffee_name} not found".format(coffee_name=coffee_name)
        )                